...
class Node:
    ...
    def __draw__(rnode, chars, v, ed='#'):
        l = len(chars)
        edges = list(rnode.getoutedges().items())
        nogc = []
        hasgc = []
        gc = []
        maxlen = len(chars) + 6
    ...
    while ind < len(chars):
        ...
        if remainder == 0:
            if actnode.getoutedges() is not None and ch in actnode.getoutedges():
                ...
                actnode.setoutedge(chars[ind], actnode, ind, '#', aleaf)
        else:
    ...
    while remainder > 0:
        ...
        if actlen == 0:
            ...
            if remains[actlen_re + actlen] != chars[start + actlen]:
                ...
                newnode.setoutedge(chars[start + actlen], newnode, start + actlen, end, bnode)
                aleaf = Node(None, None, None)
                aedge = (newnode, ind, '#', aleaf)
                aleaf.setparentkey((newnode, chars[ind]))
                newnode.setoutedge(chars[ind], newnode, ind, '#', aleaf)
            else:
                return remainder, actnode, actkey, actlen
